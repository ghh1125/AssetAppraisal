...
`frontend/` 将人工输入、三个可选材料上传、后端固定 Word 模板、GLM/企查查开关和产物下载做成页面。三个上传项固定为：审计报告 PDF、资产基础法/资产清查 Excel、收益法或市场法 Excel；只需至少上传一份材料或填写一项基础信息，工作簿支持 `.xlsx/.xlsm`。只有选择 PDF 后才检查 OCR 缓存。Word 模板由后端固定提供，不需要上传；参考报告 DOCX 和系统生成的 OCR Excel 也不属于前端输入。
