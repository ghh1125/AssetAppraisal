...
4. `output`（结果输出）：输出独立的待复核 Word，并为所有写入内容增加状态底色和来源批注。来源及审核通过为浅绿；采用 Excel 等来源但尚未完成 PDF 对照为浅橙；来源冲突、口径歧义或 LLM 复核存疑为浅红；所有材料无值时保留黄色 `XXX`、将文字标红并附缺失批注。四类批注标题由程序固定为 `【来源已核验】`、`【来源待补充核验】`、`【数据冲突，需人工复核】`、`【未找到数据】`；正文交给受限 LLM 根据已确认的人工输入、双方文件、PDF 页码、Excel 工作表、企查查 API 或百炼模型证据撰写。LLM 不得更改状态、数值和来源，调用失败时使用同证据的规则正文兜底。
