...
1. `start_input`（开始/输入）：只接收图片定义的五项人工基础信息——委托方、评估主体、委托类型（单选）、评估方法（多选且至少一个）、评估结论采用方法（单选）；资料入口支持一次上传多个 Word/Excel/PDF 文件。Word 模板始终由后端提供，不接受用户覆盖。
...
- 百炼叙述：在候选生成节点为七个固定 Word 位置生成候选文本；默认使用 `qwen3.7-max-2026-05-17`，可在项目 YAML 的 `llm.tasks.narrative` 或环境变量中修改。
...
模板批注：仓库中的 `templates/评估报告版式-沟通标注版_批注版.docx`保留了新版 Word 批注，Web 项目通过 `web_template` 默认使用它。运行时会读取批注 ID、批注文本和对应占位符，写入内部 `template_comments.json`，并优先用批注所在位置匹配旧字段映射；无批注的旧模板继续使用黄色标注兼容路径。批注只是来源说明，不会作为最终报告正文输出。
...
页面第一节点只提交图片定义的五项人工基础信息和多选资料文件。任务创建后，第二节点展示七个固定 Word 位置的 LLM 候选，用户勾选后再提交填充。企查查、百炼和 PDF OCR/XLSX 不要求用户在页面重复填写；没有列在第一节点的批注人工字段继续保留 `XXX`。
