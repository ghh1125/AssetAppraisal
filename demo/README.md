...
0. 可选预处理：上传 RAR/ZIP 审计材料包，安全解压并 OCR 全部 PDF/图片，完成材料分类、主体匹配、通用逐单元格规则读取与映射，生成可下载的 `资产法.xlsx`、`收益法.xlsx`。用户随后上传的修改版工作簿按角色逐份覆盖生成版；未上传修改版的角色继续沿用生成版。
1. `start_input`（开始/输入）：接收委托方全称/简称、委托类型、评估主体全称/简称、评估对象、评估方法、评估结论采用方法和评估基准日。预处理不是必需；用户可直接上传资产法/收益法 Excel 进入本阶段，也可沿用散装 PDF/Word/Excel 审计材料入口。工商、股权沿革、账外无形资产、企业介绍各自选择上传文件或企查查 API。报告流水号已移除。
...
4. `output`（结果输出）：输出独立的待复核 Word，并为所有写入内容增加状态底色和来源批注。来源及审核通过为浅绿；采用 Excel 等来源但尚未完成 PDF 对照为浅橙；来源冲突、口径歧义或 LLM/规则复核存疑为浅红；所有材料无值时保留黄色 `XXX`、将文字标红并附缺失批注。每张表只生成一条覆盖整个表格选择范围的审核概览；若表内存在异常，概览说明通过、不通过、待人工核对和缺失数量，已定位的异常单元格另附具体批注，正常单元格不再重复堆批注但仍独立着色。LLM 复核失败时不虚构单项失败位置，而是整表标红并将非缺失项列为待人工核对。四类批注标题由程序固定为 `【来源已核验】`、`【来源待补充核验】`、`【数据冲突，需人工复核】`、`【未找到数据】`；正文交给受限 LLM 根据已确认的人工输入、双方文件、PDF 页码、Excel 工作表、企查查 API 或百炼模型证据撰写。LLM 不得更改状态、数值和来源，调用失败时使用同证据的规则正文兜底。

节点 3 和节点 4 的每个动作都会实时写入任务状态。前端会展开显示“解析审计 PDF、解析 Excel、企查查 API 搜索、整合并比对来源、LLM 生成候选、LLM 组织审核批注、填写某个文字字段、填写财务表格、写入批注、检查 XXX”等子步骤，并显示当前动作、完成状态和最后一条进度消息。节点 3 在候选生成后暂停，节点 4 只有在人工确认候选后才开始。
...
`frontend/` 将页面整理成两个大阶段。第一阶段可选上传 RAR/ZIP，并轮询 `/api/v1/asset-appraisal/workbook-intakes/{id}` 展示 12 个实时子步骤，完成后可从产物接口下载两份 Excel。第二阶段是原有流程：若提交时携带预处理 ID，后端以生成版作为默认工作簿；同次提交的 `reporting_workbook` 或 `income_workbook` 上传文件分别覆盖对应生成版。没有预处理 ID 时，直接上传业务 Excel 或散装审计材料也能运行。Word 模板和 OCR 结构化 Excel 仍由后端管理。
...
前端显示五个业务节点：节点 1 是可选的材料包预处理；后端运行时间线显示节点 2～5。节点 3 完成后状态为“等待人工选择”，用户确认候选后才继续节点 4 和节点 5；节点状态同时写入 API 返回的 `nodes` 和 `workflow_trace.json`。
