...
- 审计 PDF 与表格同字段同期间出现冲突：报告按批注规则采用 PDF 值，并把该数值标红、增加 Word 批注；程序提供双方文件、精确位置、采用值和真实 Word 候选，由受限 LLM 选择落点并生成核对基准日/口径/单位/科目定义的中文说明。重复金额无法落到具体数字时改挂科目行名、期间表头或章节语义锚点；同一位置的多个事项合并为编号复核项，不静默丢失。
