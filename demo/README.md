...
模板批注：仓库中的 `templates/评估报告版式-沟通标注版_批注版.docx`保留了新版 Word 批注。运行时会读取批注 ID、批注文本和对应占位符，写入内部 `template_comments.json`，并优先用批注所在位置匹配旧字段映射；无批注的旧模板继续使用黄色标注兼容路径。批注只是来源说明，不会作为最终报告正文输出。
