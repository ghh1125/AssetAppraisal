...
前端只有一个“相关资料”上传入口，可一次选择多个 Word、Excel、PDF 文件；上传框按扩展名和工作表语义识别，文件名可以不同，有多少就处理多少。审计 PDF 会生成或复用 `OCR结构化结果.xlsx`；资产基础法/资产清查、收益法或市场法 Excel 直接进入语义取数；补充 Word 作为材料来源传入但不会覆盖后台模板。

人工基础信息严格对应图片中的五项：委托方、评估主体、委托类型（转让/收购/增资/减资单选）、评估方法（资产基础法/收益法/市场法多选且至少一个）、评估结论采用方法（单选）。报告流水号、评估对象等批注标记为人工来源但未列入第一节点的字段会保留 `XXX`，不会由系统臆造。
