...
2. 输入路径三选一即可：上传 RAR/ZIP 自动生成两份业务 Excel；直接上传资产法/收益法 Excel；或继续上传多个散装 PDF、Word、Excel 审计材料。
...
前端显示两条互不混淆的大进度条。第一条仅表示可选的材料包预处理，并展开上传校验、安全解压、清点、逐文件 OCR、材料分类、主体匹配、读取模板与规则、资产法映射、收益法映射、规则图构建和结果校验。第二条保留原四节点工作流的细步骤：校验输入、整理材料、OCR/Excel 解析、企查查搜索、来源整合比对、LLM 候选和批注生成、逐字段/逐表填充、写入批注、检查黄色 `XXX` 以及最终 Word 文件校验。
