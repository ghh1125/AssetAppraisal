...
可配置的资产评估报告 Demo 工作流：把审计 PDF 识别为结构化 OCR Excel，再按模板中的字段来源规则，将 PDF/OCR/XLSX、企查查 API、百炼模型和用户输入分别填入 Word，并在生成后执行格式、数据和语义审核。输出始终复制模板生成，不覆盖原模板；每次运行另存 `workflow_trace.json`，记录实际执行节点及其输入输出契约、版本、来源证据和问题。
...
生成 Word 后，若启用百炼模型，还会生成 `格式审核.json`、`数据校验.json`、`语义审核.json` 和 `审核汇总.json`。审核只提出问题和证据，不直接改写 Word；所有问题同时汇总到 `issues.json`。工作流契约不合法、模板缺失、材料损坏或模板结构错误时停止生成；企查查、LLM 或财务字段单项无结果时按既定规则留空并保留可审计轨迹。
审核完成且财务字段完整时另存 `资产评估报告_最终候选.docx`。财务字段缺失时仍生成 `资产评估报告_待复核.docx`，缺失值保持空白并列入高优先级问题清单，不生成最终候选版本。
