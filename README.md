...
生成 Word 后，若启用百炼模型，还会生成 `格式审核.json`、`数据校验.json`、`语义审核.json` 和 `审核汇总.json`。审核只提出问题和证据，不直接改写 Word；所有问题同时汇总到 `issues.json`。工作流契约不合法、模板缺失或金额必填门禁失败时停止生成；企查查或 LLM 单项不可用时按既定规则留空、标记失败或跳过，并保留可审计轨迹。
