...
节点 1 使用仓库内置、公司无关且带 SHA-256 完整性校验的版本化工作簿模板包 `demo/workbook_templates/`。模板包只保留两类工作簿的 Sheet 结构、样式、字段角色和公式链，不含通富或其他客户的公司名、金额、预测参数及业务明细；运行时按清单中的 `asset` / `income` 角色加载，不再拼接某个项目的文件名，也不读取被忽略的 `资产评估工作流/`。如需发布另一版标准模板，可复制完整模板包、更新 `workbook_templates.json` 中的文件与 SHA-256，并通过 `APPRAISAL_WORKBOOK_TEMPLATE_DIR` 指向该目录，无需修改业务代码。
...
这里的 Word 报告模板与节点 1 的 Excel 模板包职责不同：Word 模板控制最终报告版式；`demo/workbook_templates/` 控制 `资产法.xlsx`、`收益法.xlsx` 的工作簿结构与公式。两者都属于仓库内版本化应用资源，客户上传材料只提供本次案例证据。
...
当前阿里云 ECS 测试环境使用 Nginx `8002`、后端 systemd 服务 `asset-appraisal` 和后端端口 `8012`；更新、验收及回滚命令见 [测试环境部署流程](docs/测试环境部署流程.md)。
