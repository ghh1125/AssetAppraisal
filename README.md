...
可配置的资产评估报告 Demo 工作流：按现有材料读取 PDF/OCR/XLSX、企查查 API、百炼模型和用户输入，再依据模板字段来源规则填入 Word。PDF 和各类项目材料均可缺省；找不到的数据在 Word 保留黄色占位符，并输出带 Word 页码和具体位置的生成问题清单。输出始终复制模板生成，不覆盖原模板。
