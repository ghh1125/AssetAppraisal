...
test('keeps node one archive preprocessing separate from node two original inputs', () => {
  ...
  assert.match(source, /openNode2Section\('manual'\)/)
  assert.match(source, /openNode2Section\('materials'\)/)
  assert.ok(source.indexOf('archive-stage-card') < source.indexOf('node2-card'))
  const materialsModal = source.slice(source.indexOf('<a-modal v-model:open="materialsModalOpen"'), source.indexOf('</a-modal>', source.indexOf('<a-modal v-model:open="materialsModalOpen"')))
  assert.doesNotMatch(materialsModal, /materialArchive/)
  assert.doesNotMatch(materialsModal, /generateWorkbooksFromArchive/)
  assert.match(materialsModal, /workbook-routing-grid/)
  assert.match(materialsModal, /files\.reportingWorkbook/)
  assert.match(materialsModal, /files\.incomeWorkbook/)
